...
from .models import Route, RouteDelivery
from . import db
...
def dashboard():
    ...
    deliveries_today = []
    if route_today:
        deliveries_today = RouteDelivery.query.filter_by(
            route_id=route_today.route_id
        ).order_by(RouteDelivery.sequence.asc()).all()

    # Route morgen
    route_tomorrow = Route.query.filter_by(driver_id=driver_id, route_date=tomorrow).first()
    deliveries_tomorrow = []
    if route_tomorrow:
        deliveries_tomorrow = RouteDelivery.query.filter_by(
