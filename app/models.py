...
class PurchaseOrders(db.Model):
    ...
    delivery_hour_start = db.Column(db.Time)
    delivery_hour_end = db.Column(db.Time)
...
class Route(db.Model):
    ...
    def total_weight(self):
        """Som van alle gewichten in deze route."""
        if not self.deliveries:
            return 0
        return sum((d.order.total_weight_kg or 0) for d in self.deliveries)
